...
class TestScreenWrapComponent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.transform = self.entity.add_component(TransformComponent, 0.0, 0.0)
        self.wrap = self.entity.add_component(ScreenWrapComponent, 800, 600)
    
    def test_wrap_x(self):
        """Test wrapping in x direction."""
        # Test right edge
        self.transform.position[0] = 850
        self.wrap.update(0.016)
        self.assertEqual(self.transform.position[0], 0)
        
        # Test left edge
        self.transform.position[0] = -50
        self.wrap.update(0.016)
        self.assertEqual(self.transform.position[0], 800)
    
    def test_wrap_y(self):
        """Test wrapping in y direction."""
        # Test bottom edge
        self.transform.position[1] = 650
        self.wrap.update(0.016)
        self.assertEqual(self.transform.position[1], 0)
        
        # Test top edge
        self.transform.position[1] = -50
        self.wrap.update(0.016)
        self.assertEqual(self.transform.position[1], 600)
...
class TestPhysicsComponent(unittest.TestCase):
    ...
    def test_force_application(self):
        ...
        self.assertTrue(np.allclose(self.transform.velocity, expected_velocity))
    
    def test_max_speed(self):
        """Test maximum speed limit."""
        # Apply large force
        force = np.array([1000.0, 0.0])
        self.physics.apply_force(force)
        self.physics.update(1.0)
        
        speed = np.linalg.norm(self.transform.velocity)
        self.assertLessEqual(speed, self.physics.max_speed)
    
    def test_friction(self):
        """Test friction application."""
        # Set initial velocity
        self.transform.velocity = np.array([10.0, 0.0])
        self.physics.friction = 0.5  # 50% friction per update
        
        self.physics.update(1.0)
        expected_velocity = np.array([5.0, 0.0])  # After 50% friction
        self.assertTrue(np.allclose(self.transform.velocity, expected_velocity))
